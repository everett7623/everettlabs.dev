...
- Coffee 只支持 `USDT · TRON (TRC20)`、`USDC · Base`、`BTC · Bitcoin` 和 `USDT · TON`，资产与网络必须成对显示，禁止继续扩展重复网络或提供含糊的跨链地址。
- 只接收和记录公开收款地址；不得请求、存储或提交私钥、助记词、Keystore、交易所密码、验证码或 API 密钥。
- Coffee 当前使用用户确认的 Binance 公开充值地址；不得自行迁移到其他钱包或收款平台，地址变更时必须重新核对资产、网络和完整地址。
