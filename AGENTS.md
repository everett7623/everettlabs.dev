...
3. `.codex/tasks/*.md`：活跃迭代的状态、待办、阻塞和验证记录；已完成任务按月份归档到 `.codex/tasks/archive/`。
...
- Coffee 支持官方 Ko-fi 页面 `https://ko-fi.com/everettlabs`，以及 `USDT · TRON (TRC20)`、`USDC · Base`、`BTC · Bitcoin` 和 `USDT · TON` 四种链上方式；资产与网络必须成对显示，禁止继续扩展重复网络或提供含糊的跨链地址。
- 只接收和记录公开收款地址；不得请求、存储或提交私钥、助记词、Keystore、交易所密码、验证码或 API 密钥。
- Coffee 的链上方式当前使用用户确认的 Binance 公开充值地址；Ko-fi 不替换这些地址，不得自行迁移到其他钱包或收款平台，地址变更时必须重新核对资产、网络和完整地址。
- Ko-fi 只使用指向官方页面的普通 HTTPS 外链；不得嵌入第三方脚本、iframe、远程图片、Cookie 或未经批准的分析事件。
- 收款地址为空时不渲染支付卡片；非空地址必须通过对应网络的地址验证，支持校验和的地址格式必须验证校验和；二维码本地生成，复制操作需提供键盘支持和可感知状态。
...
- GEO 在本项目中指生成式搜索可发现性，不是独立排名保证；继续以可抓取、可索引、对用户有用的
  静态文本和基础 SEO 为准，不创建只向爬虫展示的内容。
- `robots.txt` 必须允许普通搜索爬虫和 `OAI-SearchBot` 访问公开页面；`GPTBot` 等训练爬虫属于
  独立的隐私与产品决策，未经用户明确要求不得混同修改。
- `public/llms.txt` 是补充性机器可读索引，必须包含核心页面和全部批准项目，并遵守人工白名单、
  归属、风险提示与禁用内容边界；不得把它描述为 Google、OpenAI 或 Schema.org 的正式排名标准。
- JSON-LD、canonical、Open Graph、sitemap 和 `llms.txt` 必须与页面可见内容一致。禁止为了 GEO
  虚构 FAQ、评分、评价、用户数量、发布日期、维护状态或项目能力。
- 新增、删除或重命名公开页面和项目时，必须同步检查 sitemap、robots、结构化数据、内部链接、
  `llms.txt` 与 `validate:site` 契约。
...
- `validate:projects` 必须确认所有公开项目均在批准白名单内。
